...
display.fictiveRect = nan; % no fictive feedback
...
while ((GetSecs-runOnset)<sessSecs) && (trialNum<=trialLimit) % proceed continuously until time is up
    ...
    elseif strcmp(trialPhase,'waiting')
        ...
        if detectResponse(params.respChar)
            ...
            %%% FICTIVE FEEDBACK %%%
            % if applicable, display the scheduled reward time
            % mark a slash on the progress bar at 'waitDuration'
            if display.fictive % && strcmp(display.tokenState,'low')
                % identify the screen position of the scheduled reward time
                fictive_xPos = rects.timeBar(1) + ceil(rects.timeBarMax*waitDuration/display.timeBarMaxTime);
                % define a rect at that location
                display.fictiveRect = [fictive_xPos-2, rects.timeBar(2)-5, fictive_xPos+2, rects.timeBar(4)+5];
            end % if-statement controlling fictive feedback
    ...
    elseif strcmp(trialPhase,'feedback') && eventLatency>(display.iti)/2
        ...
        display.tokenState = 'low';
        display.soldMsg = false;
        display.fictiveRect = nan; % remove fictive feedback if any
        changeDisp = true;
    end
