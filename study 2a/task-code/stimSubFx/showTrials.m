...
%
% modified for the wtw_discrete_implicit experiment:
%   allows for a response deadline after which the matured token expires
%   allows for disallowing quitting & penalizing anticipatory responses
...
responseDeadline = params.responseDeadline; % in s; nan means no deadline
prohibitQuitting = params.prohibitQuitting; % penalty in cents; nan means quitting is permitted
...
% show trials
% each trial consists of 3 phases:
...
itiKeypressTimes = []; % to log keypresses during ITI/feedback
anticKeypressTimes = []; % to log anticipatory keypresses (if quitting is disallowed)
while ((GetSecs-runOnset)<sessSecs) && (trialNum<=trialLimit) % proceed continuously until time is up
    ...
    elseif strcmp(trialPhase,'waiting')
        ...
        % detect if the key is down
        dr = detectResponse(params.respChar);

        % detect if the key has been newly released
        if keyDown && ~dr % key was just released
            keyDown = false;
        end
        
        % detect if the key has been newly pressed
        respNow = false;
        if dr && ~keyDown
            respNow = true;
            keyDown = true;
        ...
        % if quitting is disallowed, penalize and ignore quits
        if respNow && ~isnan(prohibitQuitting) && strcmp(display.tokenState,'low')
            display.totalWon = display.totalWon - prohibitQuitting;
            anticKeypressTimes = [anticKeypressTimes, timeNow-phaseOnset]; %#ok<AGROW>
            continue; % skips code below so trial will not end
        end
        
        % determine whether the trial has expired
        trialExpired = false;
        if ~isnan(responseDeadline) && strcmp(display.tokenState,'high') 
            timeSinceRwd = timeNow - phaseOnset - rwdTime;
            if timeSinceRwd>responseDeadline
                trialExpired = true;
            end
        end
        
        % unique tasks if the token has expired (but not been sold)
        if trialExpired
            earnings = 0;
            display.expireMsg = true;
        end
        
        % unique tasks if the token has been sold (but not expired)
        if respNow
            earnings = display.tokenValue;
            display.soldMsg = true;
        end
        
        % end the trial if the token is sold OR expired
        % change phase: "waiting" -> "feedback"
        % a line is added to the data record here
        if respNow || trialExpired
            trialPhase = 'feedback';
            phaseOnset = timeNow;
            ...
            trialData(datarow).itiKeypresses = itiKeypressTimes;
                ...
                % matrix.
            trialData(datarow).anticKeypresses = anticKeypressTimes;
                % likewise for anticipatory keypresses if quitting is
                % disallowed
            ...
            trialData(datarow).trialExpired = trialExpired; % whether the current trial expired
            ...
            % 10=total earnings
            fprintf(datafid,'%d,%d,%1.3f,%d,%d,%1.3f,%1.3f,%1.3f,%1.3f,%d,%d,%d\n',...
                block,trialNum,initialTime,any(itiKeypressTimes),numel(anticKeypressTimes),...
                waitDuration,rwdTime,eventLatency,sellTime,trialExpired,earnings,...
                display.totalWon);
            
            % reinitialize record of ITI (and anticipatory) keypress times
            itiKeypressTimes = [];
            anticKeypressTimes = [];
